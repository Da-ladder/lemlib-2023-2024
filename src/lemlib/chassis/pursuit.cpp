...
std::vector<lemlib::Pose> getData(std::string filePath) {
    ...
    while (getline(file, line) && line != "endData") {
        ...
        pathPoint.theta = std::stof(pointInput.at(2)); // velocity/angle?
...
"-17.087, -22.168, 87.528",
"-17.062, -20.168, 84.577",
"-17.007, -18.169, 81.519",
"-16.916, -16.171, 78.341",
"-16.779, -14.176, 75.029",
"-16.578, -12.186, 71.563",
"-16.319, -10.203, 67.922",
"-15.99, -8.23, 64.074",
"-15.581, -6.273, 59.979",
"-15.081, -4.337, 55.584",
"-14.464, -2.435, 50.81",
...
std::vector<lemlib::Pose> getDataFromList(std::vector<std::string>* pointList) {
    std::vector<lemlib::Pose> robotPath;
    std::vector<std::string> pointInput;
    lemlib::Pose pathPoint(0, 0, 0);

    // read the points until 'endData' is read
    for (std::string fullPoint : *pointList) {
        pointInput = readElement(fullPoint, ", "); // parse line
        pathPoint.x = std::stof(pointInput.at(0)); // x position
        pathPoint.y = std::stof(pointInput.at(1)); // y position
        pathPoint.theta = std::stof(pointInput.at(2)); // velocity/angle?
        robotPath.push_back(pathPoint); // save data
 ...
 * @param log whether the chassis should log the path on a log file. false by default.
 */
void lemlib::Chassis::followFromVector(std::vector<std::string>* pointList, int timeout, float lookahead, bool reverse, float maxSpeed,
                             bool log) {
    std::vector<lemlib::Pose> path = getDataFromList(pointList); // get list of path points
    Pose pose(0, 0, 0);
    Pose lookaheadPose(0, 0, 0);
    Pose lastLookahead = path.at(0);
    lastLookahead.theta = 0;
